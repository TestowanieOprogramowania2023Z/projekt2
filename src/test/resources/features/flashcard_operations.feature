Feature: Managing Flashcards in the Web Service
  Scenario: User gets all existing flashcards from a existing set by non empty set name
    Given there is a set with the name "Set 1"
    And there are flashcards in the set with the name "Set 1"
    When the user requests all flashcards from the set with the name "Set 1"
    Then the user gets all flashcards from the set with the name "Set 1"
